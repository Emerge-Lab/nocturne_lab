...
def poly_intersection(poly1, poly2):
    ...
    for i, p1_first_point in enumerate(poly1[:-1]):
        ...
        for j, p2_first_point in enumerate(poly2[:-1]):
            ...
            if intersect(p1_first_point, p1_second_point, p2_first_point, p2_second_point):
                return True
...
def run_rollouts(
    ...
    if actor_2 is not None:
        ...
        indices_1 = list(np.random.choice(valid_indices, num_choice, replace=False))
        ...
        rnn_states_2 = torch.zeros([env.num_agents, get_hidden_size(cfg)], dtype=torch.float32, device=device)
    else:
        rnn_states = torch.zeros([env.num_agents, get_hidden_size(cfg)], dtype=torch.float32, device=device)
    ...
    while not all(finished_episode):
        with torch.no_grad():
            ...
            if actor_2 is not None:
                ...
                policy_outputs_2 = actor_2(obs_torch_2, rnn_states_2, with_action_distribution=True)

            policy_outputs = actor_1(obs_torch, rnn_states, with_action_distribution=True)
            ...
            if isinstance(action_distribution, ContinuousActionDistribution):
                if not cfg.continuous_actions_sample:  # TODO: add similar option for discrete actions
                    ...
                    if actor_2 is not None:
                        actions[indices_2] = policy_outputs_2.action_distribution.means[indices_2]
            if isinstance(action_distribution, CategoricalActionDistribution):
                if not cfg.discrete_actions_sample:
                    ...
                    if actor_2 is not None:
                        actions[indices_2] = policy_outputs_2["action_logits"].argmax(axis=1)[indices_2]
            ...
            for veh in env.unwrapped.get_objects_that_moved():
                ...
                if veh.expert_control is False:
                    rollout_traj_dict[veh.id][env.step_num] = veh.position.numpy()
            ...
            for i, index in enumerate(valid_indices):
                goal_achieved[i] = infos[index]["goal_achieved"] or goal_achieved[i]
                collision_observed[i] = infos[index]["collided"] or collision_observed[i]
            ...
            if all(finished_episode):
                ...
                avg_veh_edge_collisions = infos[0]["episode_extra_stats"]["veh_edge_collision"]
                avg_veh_veh_collisions = infos[0]["episode_extra_stats"]["veh_veh_collision"]
                success_rate_by_num_agents[len(valid_indices) - 1, 0] += avg_goal
                success_rate_by_num_agents[len(valid_indices) - 1, 1] += avg_collisions
                success_rate_by_num_agents[len(valid_indices) - 1, 2] += np.mean(veh_veh_collision_observed)
                ...
                for i, index in enumerate(valid_indices):
                    ...
                    success_rate_by_distance[bin - 1, :] += [
                        goal_achieved[i],
                        collision_observed[i],
                        veh_veh_collision_observed[i],
                        1,
                    ]
                # track how well we do as number of intersections
                for i, index in enumerate(valid_indices):
                    env_id = agent_id_to_env_id_map[index]
                    bin = min(veh_intersection_dict[env_id], distance_bins.shape[-1] - 1)
                ...
                for agent_id, traj in rollout_traj_dict.items():
                    ...
                    mask = (masking_arr != 0.0) * (masking_arr != traj.shape[1] * ERR_VAL)
                    expert_mask_arr = expert_trajectory_dict[agent_id].sum(axis=1)
                    expert_mask = (expert_mask_arr != 0.0) * (expert_mask_arr != traj.shape[1] * ERR_VAL)
                    ade = np.linalg.norm(traj - expert_trajectory_dict[agent_id], axis=-1)[mask * expert_mask]
                    ades.append(ade.mean())
                    fde = np.linalg.norm(traj - expert_trajectory_dict[agent_id], axis=-1)[
                        np.max(np.argwhere(mask * expert_mask))
                    ]
                    fdes.append(fde)
                    veh_counter += 1
                ...
                return (
                    avg_goal,
                    avg_collisions,
                    avg_veh_edge_collisions,
                    avg_veh_veh_collisions,
...
def run_eval(cfgs, test_zsc, output_path, scenario_dir, files, file_type, device="cuda"):
    ...
    if test_zsc:
        goal_array = np.zeros((len(actor_critics), len(actor_critics), num_file_loops * num_files))
        collision_array = np.zeros((len(actor_critics), len(actor_critics), num_files * num_file_loops))
        success_rate_by_num_agents = np.zeros((len(actor_critics), len(actor_critics), cfg.max_num_vehicles, 4))
        success_rate_by_distance = np.zeros((len(actor_critics), len(actor_critics), distance_bins.shape[-1], 4))
        success_rate_by_intersections = np.zeros(
            (len(actor_critics), len(actor_critics), intersections_bins.shape[-1], 4)
        )
        ade_array = np.zeros((len(actor_critics), len(actor_critics), num_file_loops * num_files))
        fde_array = np.zeros((len(actor_critics), len(actor_critics), num_file_loops * num_files))
        veh_veh_collision_array = np.zeros((len(actor_critics), len(actor_critics), num_file_loops * num_files))
        veh_edge_collision_array = np.zeros((len(actor_critics), len(actor_critics), num_file_loops * num_files))
    else:
        goal_array = np.zeros((len(actor_critics), num_file_loops * num_files))
        collision_array = np.zeros((len(actor_critics), num_file_loops * num_files))
        veh_veh_collision_array = np.zeros((len(actor_critics), num_file_loops * num_files))
        veh_edge_collision_array = np.zeros((len(actor_critics), num_file_loops * num_files))
        success_rate_by_num_agents = np.zeros((len(actor_critics), cfg.max_num_vehicles, 4))
        success_rate_by_distance = np.zeros((len(actor_critics), distance_bins.shape[-1], 4))
        success_rate_by_intersections = np.zeros((len(actor_critics), intersections_bins.shape[-1], 4))
    ...
    for output in output_generator:
            ...
            for file_num, file in enumerate(files[0 : cfg["num_eval_files"]]):
                ...
                for veh_id in expert_trajectory_dict.keys():
                    for veh_id2 in expert_trajectory_dict.keys():
                        ...
                        expert_mask = (expert_mask_arr != 0.0) * (expert_mask_arr != trajectory.shape[1] * ERR_VAL)
                        trajectory = trajectory[expert_mask]
                        expert_mask_arr = trajectory2.sum(axis=1)
                        expert_mask = (expert_mask_arr != 0.0) * (expert_mask_arr != trajectory2.shape[1] * ERR_VAL)
                        trajectory2 = trajectory2[expert_mask]
                        if poly_intersection(trajectory, trajectory2):
                            vehs_with_intersecting_ids[veh_id] += poly_intersection(trajectory, trajectory2)
                ...
                (
                    ...
                    avg_veh_edge_collisions,
                    avg_veh_veh_collisions,
                    success_rate_by_distance_return,
                    success_rate_by_num_agents_return,
                ...
                if test_zsc:
                    success_rate_by_distance[index_1, index_2] += success_rate_by_distance_return
                    success_rate_by_num_agents[index_1, index_2] += success_rate_by_num_agents_return
                    success_rate_by_intersections[index_1, index_2] += success_rate_by_intersections_return
                else:
                    success_rate_by_distance[index_1] += success_rate_by_distance_return
                    success_rate_by_num_agents[index_1] += success_rate_by_num_agents_return
                    success_rate_by_intersections[index_1] += success_rate_by_intersections_return
                # do some logging
                log.info(f"Avg goal achieved {np.mean(goal_frac)}±{np.std(goal_frac) / len(goal_frac)}")
                log.info(
                ...
                log.info(
                    f"Avg num collisions {np.mean(collision_frac)}±\
                ...
                log.info(
                    f"Avg ADE {np.mean(average_displacement_error)}±\
                        {np.std(average_displacement_error) / np.sqrt(len(average_displacement_error))}"
                )
                log.info(
                    f"Avg FDE {np.mean(final_displacement_error)}±\
        ...
        if test_zsc:
            ...
            veh_edge_collision_array[index_1, index_2] = veh_edge_collision_frac
    ...
    if test_zsc:
        file_type += "_zsc"
    np.save(os.path.join(output_path, "{}_goal.npy".format(file_type)), goal_array)
    np.save(os.path.join(output_path, "{}_collision.npy".format(file_type)), collision_array)
    np.save(os.path.join(output_path, "{}_veh_veh_collision.npy".format(file_type)), veh_veh_collision_array)
    np.save(os.path.join(output_path, "{}_veh_edge_collision.npy".format(file_type)), veh_edge_collision_array)
    np.save(os.path.join(output_path, "{}_ade.npy".format(file_type)), ade_array)
    np.save(os.path.join(output_path, "{}_fde.npy".format(file_type)), fde_array)
    with open(os.path.join(output_path, "{}_success_by_veh_number.npy".format(file_type)), "wb") as f:
        np.save(f, success_rate_by_num_agents)
    with open(os.path.join(output_path, "{}_success_by_dist.npy".format(file_type)), "wb") as f:
        np.save(f, success_rate_by_distance)
    with open(os.path.join(output_path, "{}_success_by_num_intersections.npy".format(file_type)), "wb") as f:
...
def load_wandb(experiment_name, cfg_filter, force_reload=False):
    ...
    ----
        ...
        force_reload (bool, optional): if true we overwrite
                                       ...
                                       even if it exists.
    """
    if not os.path.exists("wandb_{}.csv".format(experiment_name)) or force_reload:
        import wandb
...
def plot_goal_achieved(experiment_name, global_step_cutoff=3e9):
    ...
    for num_files in values_num_files:
        ...
        df_n = (
            df[(df.num_files == num_files) & (df.global_step < global_step_cutoff)]
        ...
        if num_files == -1:
            ...
            col_name = num_files
        dfs.append((df_n[column] * 100).ewm(halflife=500, min_periods=10).mean().rename(f"num_files={col_name}"))
        stdevs.append((df_n[column] * 100).ewm(halflife=500, min_periods=10).std())

    values_num_files = [val if val != -1 else 134453 for val in values_num_files]
    ...
    for i in range(len(dfs)):
        ...
        ax.plot(x, y, label=f"Training Files: {values_num_files[i]}", color=CB_color_cycle[i])
        ax.fill_between(x, y - 2 * yerr, y + 2 * yerr, color=CB_color_cycle[i], alpha=0.3)
...
def eval_generalization(
    output_folder, num_eval_files, files, file_type, scenario_dir, num_file_loops, test_zsc=False, cfg_filter=None
):
    ...
    for dirpath, dirnames, filenames in os.walk(output_folder):
        if "cfg.json" in filenames:
            ...
            if cfg_dict["num_files"] < num_eval_files and "train" in file_type and cfg_dict["num_files"] != -1:
                cfg_dict["num_eval_files"] = cfg_dict["num_files"]
                cfg_dict["num_file_loops"] = num_file_loops * int(max(num_eval_files // cfg_dict["num_files"], 1))
            else:
...
def main():
    ...
    output_folder = ["/checkpoint/eugenevinitsky/nocturne/sweep/2022.06.01/srt_v27/17.35.33"]
    generalization_dfs = []
    ...
    if TEST_ZSC:
        ...
        def cfg_filter(cfg_dict):
            if (
                cfg_dict["scenario"]["road_edge_first"] is False
                and cfg_dict["scenario"]["max_visible_road_points"] == 500
                and cfg_dict["algorithm"]["encoder_hidden_size"] == 256
                and cfg_dict["num_files"] == 10000
            ):
                return True
            else:
                return False
    ...
    if RUN_EVAL:
        if TEST_ZSC:
            output_generator = [(PROCESSED_VALID_NO_TL, "test_{}".format(VERSION))]
        else:
            output_generator = [
    ...
    if PLOT_RESULTS:
            ...
            for file_type in [
                ...
                for dirpath, dirnames, filenames in os.walk(folder):
                    if "cfg.json" in filenames:
                        ...
                        if cfg_filter(cfg_dict):
                            ...
                            goal = np.mean(np.load(os.path.join(dirpath, "{}_goal.npy".format(file_type))))
                            collide = np.mean(np.load(os.path.join(dirpath, "{}_collision.npy".format(file_type))))
                            ade = np.mean(np.load(os.path.join(dirpath, "{}_ade.npy".format(file_type))))
                            fde = np.mean(np.load(os.path.join(dirpath, "{}_fde.npy".format(file_type))))
                            veh_veh_collision = np.mean(
                                np.load(os.path.join(dirpath, "{}_veh_veh_collision.npy".format(file_type)))
                            )
                            veh_edge_collision = np.mean(
                                np.load(os.path.join(dirpath, "{}_veh_edge_collision.npy".format(file_type)))
                            )
                            success_by_num_intersections = np.load(
                                os.path.join(dirpath, "{}_success_by_num_intersections.npy".format(file_type))
                            )
                            # there aren't a lot of data points past 3
                            # so just bundle them in
                            success_by_num_intersections[:, 3, :] = success_by_num_intersections[:, 3:, :].sum(axis=1)
                            success_by_num_intersections = success_by_num_intersections[:, 0:4, :]
                            success_by_veh_num = np.load(
                                os.path.join(dirpath, "{}_success_by_veh_number.npy".format(file_type))
                            )
                            success_by_distance = np.load(
                                os.path.join(dirpath, "{}_success_by_dist.npy".format(file_type))
                            )
                            ...
                            data_dicts.append(
                                {
                                    "num_files": num_files,
                                    "goal_rate": goal * 100,
                                    "collide_rate": collide * 100,
                                    "ade": ade,
                                    "fde": fde,
                                    "veh_veh_collision": veh_veh_collision,
                                    "veh_edge_collision": veh_edge_collision,
                                    "goal_by_intersections": np.nan_to_num(
                                        success_by_num_intersections[0, :, 0] / success_by_num_intersections[0, :, 3]
                                    ),
                                    "collide_by_intersections": np.nan_to_num(
                                        success_by_num_intersections[0, :, 1] / success_by_num_intersections[0, :, 3]
                                    ),
                                    "goal_by_vehicle_num": np.nan_to_num(
                                        success_by_veh_num[0, :, 0] / success_by_veh_num[0, :, 3]
                                    ),
                                    "collide_by_vehicle_num": np.nan_to_num(
                                        success_by_veh_num[0, :, 1] / success_by_veh_num[0, :, 3]
                                    ),
                                    "goal_by_distance": np.nan_to_num(
                                        success_by_distance[0, :, 0] / success_by_distance[0, :, 3]
                                    ),
                                    "collide_by_distance": np.nan_to_num(
                                        success_by_distance[0, :, 1] / success_by_distance[0, :, 3]
                                    ),
                                }
                            )
                            if cfg_dict["num_files"] == 10000:
                                print("goal ", success_by_num_intersections[0, :, 0])
                                print("num vehicles in bin", success_by_num_intersections[0, :, 3])
                df = pd.DataFrame(data_dicts)
                new_dict = {}
                for key in data_dicts[0].keys():
                    if key == "num_files":
                        continue
                    new_dict[key] = df.groupby(["num_files"])[key].mean().reset_index()
                    try:
                        new_dict[key + "_std"] = (
                            df.groupby(["num_files"])[key].std().reset_index().rename(columns={key: key + "_std"})
                        )
                    except ValueError:
                        ...
                        for name, group in df.groupby(["num_files"])[key]:
                            ...
                            std_err = np.std(np_arr, axis=0) / np.sqrt(np_arr.shape[0])
                            temp_dict[name] = std_err
                        new_dict[key + "_stderr"] = (
                            pd.Series(data=temp_dict)
                            .reset_index()
                            .rename(columns={"index": "num_files", 0: key + "_stderr"})
                        )
                ...
                for key, value in new_dict.items():
                    if key == first_elem_key:
                        continue
                    first_elem = first_elem.merge(value, how="inner", on="num_files")
        ...
        for experiment_name in experiment_names:
            ...
            training_dfs.append(pd.read_csv("wandb_{}.csv".format(experiment_name)))

        num_seeds = len(np.unique(training_dfs[0].seed))
        # create the goal plot
        plt.figure(dpi=300)
        for i, (df, file_type) in enumerate(zip(generalization_dfs, ["Train", "Test"])):
            plt.plot(np.log10(df.num_files), df.goal_rate, color=CB_color_cycle[i], label=file_type)
            ax = plt.gca()
            yerr = df.goal_rate_std.replace(np.nan, 0) / np.sqrt(num_seeds)
            ax.fill_between(
                np.log10(df.num_files),
                df.goal_rate - 2 * yerr,
                df.goal_rate + 2 * yerr,
        ...
        plt.figure(dpi=300)
        for i, (df, file_type) in enumerate(zip(generalization_dfs, ["Train", "Test"])):
            plt.plot(np.log10(df.num_files), df.collide_rate, color=CB_color_cycle[i], label=file_type)
            ax = plt.gca()
        ...
        for i, (df, file_type) in enumerate(zip(generalization_dfs, ["Train", "Test"])):
            ...
            plt.plot(np.log10(df.num_files), df.ade, label=file_type, color=CB_color_cycle[i])
            ax = plt.gca()
            ax.fill_between(
                np.log10(df.num_files), df.ade - 2 * yerr, df.ade + 2 * yerr, color=CB_color_cycle[i], alpha=0.3
            )
        ...
        for i, (df, file_type) in enumerate(zip(generalization_dfs, ["Train", "Test"])):
            ...
            plt.plot(np.log10(df.num_files), df.fde, label=file_type, color=CB_color_cycle[i])
            ax = plt.gca()
            ax.fill_between(
                np.log10(df.num_files), df.fde - 2 * yerr, df.fde + 2 * yerr, color=CB_color_cycle[i], alpha=0.3
            )
        ...
        for i, (df, file_type) in enumerate(zip(generalization_dfs, ["Train", "Test"])):
            ...
            for value in values_num_files:
                if value != 10000:
                    continue
                numpy_arr = df[df.num_files == value]["goal_by_intersections"].to_numpy()[0]
                temp_df = pd.DataFrame(numpy_arr).melt()
                plt.plot(temp_df.index, temp_df.value * 100, label=file_type, color=CB_color_cycle[i])
                numpy_arr = df[df.num_files == value]["goal_by_intersections_stderr"].to_numpy()[0]
        ...
        plt.legend()
        plt.savefig("goal_v_intersection.png", bbox_inches="tight", pad_inches=0.1)
        ...
        for i, (df, file_type) in enumerate(zip(generalization_dfs, ["Train", "Test"])):
            ...
            for value in values_num_files:
                if value != 10000:
                    continue
                numpy_arr = df[df.num_files == value]["collide_by_intersections"].to_numpy()[0]
                temp_df = pd.DataFrame(numpy_arr).melt()
                plt.plot(temp_df.index, temp_df.value * 100, color=CB_color_cycle[i], label=file_type)
                numpy_arr = df[df.num_files == value]["collide_by_intersections_stderr"].to_numpy()[0]
                std_err_df = pd.DataFrame(numpy_arr).melt()
                ax = plt.gca()
                ax.fill_between(
                    temp_df.index,
        ...
        ax.set_xticks([i for i in range(numpy_arr.shape[-1])])
        plt.legend()
        plt.savefig("collide_v_intersection.png", bbox_inches="tight", pad_inches=0.1)
        ...
        for i, (df, file_type) in enumerate(zip(generalization_dfs, ["Train", "Test"])):
            ...
            for value in values_num_files:
                if value != 10000:
                    continue
                numpy_arr = df[df.num_files == value]["goal_by_vehicle_num"].to_numpy()[0]
                temp_df = pd.DataFrame(numpy_arr).melt()
                plt.plot(temp_df.index, temp_df.value * 100, label=file_type, color=CB_color_cycle[i])
                numpy_arr = df[df.num_files == value]["goal_by_vehicle_num_stderr"].to_numpy()[0]
                std_err_df = pd.DataFrame(numpy_arr).melt()
                ax = plt.gca()
                ax.fill_between(
                    temp_df.index,
        ...
        ax.set_xticks([i for i in range(numpy_arr.shape[-1])])
        plt.legend()
        plt.savefig("goal_v_vehicle_num.png", bbox_inches="tight", pad_inches=0.1)
        ...
        for i, (df, file_type) in enumerate(zip(generalization_dfs, ["Train", "Test"])):
            ...
            for value in values_num_files:
                if value != 10000:
                    continue
                numpy_arr = df[df.num_files == value]["goal_by_distance"].to_numpy()[0]
                temp_df = pd.DataFrame(numpy_arr).melt()
                plt.plot(temp_df.index, temp_df.value * 100, label=file_type, color=CB_color_cycle[i])
                numpy_arr = df[df.num_files == value]["goal_by_distance_stderr"].to_numpy()[0]
                std_err_df = pd.DataFrame(numpy_arr).melt()
                ax = plt.gca()
                ax.fill_between(
                    temp_df.index,
