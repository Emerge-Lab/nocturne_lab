...
def get_sim(scenario_file, cfg):
    ...
    sim = Simulation(scenario_path=str(scenario_file), config=get_scenario_dict(cfg))
    for obj in sim.getScenario().getObjectsThatMoved():
        obj.expert_control = True
    return sim


def make_movie(sim, scenario_fn, output_path="./vid.mp4", dt=0.1, steps=90, fps=10):
    ...
    for file in files:
        ...
        if os.path.exists(file):
            ...
            make_image(
                ...
                output_path=PROJECT_PATH
                / "scripts/paper_plots/figs/cone_original_{}.png".format(os.path.basename(file)),
            )
            make_image(
                sim,
                file,
                ...
                output_path=PROJECT_PATH / "scripts/paper_plots/figs/cone_{}.png".format(os.path.basename(file)),
            )
            make_image(
                sim,
                file,
                ...
                output_path=PROJECT_PATH / "scripts/paper_plots/figs/feature_{}.png".format(os.path.basename(file)),
            )
