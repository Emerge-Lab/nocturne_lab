    ...
    for i in range(len(agent_indices)):
        for j in range(len(agent_indices)):
            if np_arr_mean[i, j] > white_switch:
                ...
                color = "w"
            ax.text(j, i, f"{np.round(np_arr_mean[i, j], decimals=2)}", ha="center", va="center", color=color)
...
def compute_average_change(file):
    ...
    cross_play = np.mean(np_arr_mean[np.where(~np.eye(np_arr_mean.shape[0], dtype=bool))])
    self_play_std = np.std(np.diag(np_arr_mean)) / np.sqrt(np_arr_mean.shape[0])
    cross_play_std = np.std(np_arr_mean[np.where(~np.eye(np_arr_mean.shape[0], dtype=bool))]) / np.sqrt(
        np_arr_mean.shape[0] ** 2 - np_arr_mean.shape[0]
    )
    print(f"self play: {self_play} ± {self_play_std}, cross play: {cross_play} ± {cross_play_std}")
...
if __name__ == "__main__":
    ...
    create_heat_map("train_zsc_goal.npy", "Cross-play Goal Rate", "cross_play_heat_map.png", white_switch=0.8)
    create_heat_map(
        "train_zsc_collision.npy", "Cross-play Collision Rate", "cross_play_collision_map.png", white_switch=0.18
    )
