...
if __name__ == "__main__":
    ...
    for i, (cfg_dict, model) in enumerate(zip(cfg_dicts, models)):
        ade, fde, collisions, goals = compute_average_displacement(PROCESSED_VALID_NO_TL, model=model, configs=cfg_dict)
        results[i, 0] = ade[0]
        results[i, 1] = ade[1]
        results[i, 2] = fde[0]
        results[i, 3] = fde[1]
        ...
        results[i, 6] = goals[0]
        results[i, 7] = goals[1]
    np.save(os.path.join(PROJECT_PATH, "scripts/paper_plots/il_results.npy"), results)
    print(f"ade {np.mean(results[:, 0])} ± {np.std(results[:, 0]) / np.sqrt(results[:, 0].shape[0])}")
    print(f"fde {np.mean(results[:, 2])} ± {np.std(results[:, 2]) / np.sqrt(results[:, 0].shape[0])}")
    print(f"collisions {np.mean(results[:, 4])} ± {np.std(results[:, 4]) / np.sqrt(results[:, 0].shape[0])}")
    print(f"goals {np.mean(results[:, 6])} ± {np.std(results[:, 6]) / np.sqrt(results[:, 0].shape[0])}")
