...
def test_rl_env():
    ...
    _ = env.scenario.getConeImage(
        env.scenario.getVehicles()[0], 80.0, 120 * (np.pi / 180), 0.0, draw_target_position=False
    )
    for _ in range(90):
        vehs = env.scenario.getObjectsThatMoved()
        prev_position = {veh.getID(): [veh.position.x, veh.position.y] for veh in vehs}
        t = time.perf_counter()
        obs, rew, done, info = env.step({veh.id: Action(acceleration=2.0, steering=1.0) for veh in vehs})
        times.append(time.perf_counter() - t)
        for veh in vehs:
            if veh in env.scenario.getObjectsThatMoved():
                new_position = [veh.position.x, veh.position.y]
                assert (
                    prev_position[veh.getID()] != new_position
                ), f"veh {veh.getID()} was in position \
