...
from cfgs.config import ERR_VAL, PROCESSED_VALID_NO_TL
...
def _average_displacement_impl(arg):
    ...
    for i in range(n_stacked_states - 1):
        for veh in controlled_vehicles:
            ...
            visible_state = scenario_model.flattened_visible_state(veh, view_dist=view_dist, view_angle=view_angle)
            state = np.concatenate((ego_state, visible_state)) / state_normalization
            state_dict[veh.getID()] = np.roll(state_dict[veh.getID()], len(state))
    ...
    for i in range(SIM_N_STEPS - n_stacked_states):
            ...
            state = (
                np.concatenate(
                    (
                        ...
                        scenario_model.flattened_visible_state(veh, view_dist=view_dist, view_angle=view_angle),
                    )
                )
                / state_normalization
            )
        ...
        all_actions = model(all_states, deterministic=True)  # /!\ this returns an array (2,n) and not (n,2)
        accel_actions = all_actions[0].cpu().numpy()
        steering_actions = all_actions[1].cpu().numpy()
        # set vehicles actions
        for veh, accel_action, steering_action in zip(controlled_vehicles, accel_actions, steering_actions):
        ...
        for i, veh in enumerate(controlled_vehicles):
            ...
            if np.isclose(expert_veh.position.x, ERR_VAL) or expert_veh.collided:
                continue
    ...
    avg_displacement = np.mean(avg_displacements) if len(avg_displacements) > 0 else np.nan
    final_displacement = np.mean(final_displacements) if len(final_displacements) > 0 else np.nan
...
def compute_average_displacement(trajectories_dir, model, configs):
    ...
    with Pool(processes=14) as pool:
        result = list(pool.map(_average_displacement_impl, zip(trajectories_paths, repeat(model), repeat(configs))))
        ...
        print(average_displacements, final_displacements, average_collisions, average_goals)
    ...
    return (
        ...
        [np.mean(average_goals[~np.isnan(average_goals)]), np.std(average_goals[~np.isnan(average_goals)])],
    )
...
if __name__ == "__main__":
    ...
    with open(
        ...
        configs["device"] = "cpu"
    with torch.no_grad():
        ade, fde, collisions, goals = compute_average_displacement(PROCESSED_VALID_NO_TL, model=model, configs=configs)
    ...
    print(f"Average Success at getting to goal: {goals[0]:.3f} ± {goals[1]:.3f}%")
