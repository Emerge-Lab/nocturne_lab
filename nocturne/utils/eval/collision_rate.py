...
def _collision_rate_impl(trajectory_path, model=None, sim_allow_non_vehicles=True, check_vehicles_only=True):
    ...
    objects_to_check = [
        obj
        for obj in (vehicles_that_moved if check_vehicles_only else objects_that_moved)
...
def compute_average_collision_rate(trajectories_dir, model=None, **kwargs):
    ...
    average_collision_rates = np.array(
        list(map(lambda path: _collision_rate_impl(path, model, **kwargs), trajectories_paths))
    )
...
if __name__ == "__main__":
    ...
    collisions_with_vehicles, collisions_with_road_lines = compute_average_collision_rate(
