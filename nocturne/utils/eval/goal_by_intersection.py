...
def _compute_expert_intersections(trajectory_path):
    ...
    for veh1, veh2 in combinations(segments.keys(), 2):
        ...
        idx1 = np.repeat(np.arange(len(segments1)), len(segments2))  # build indexes 1 1 1 2 2 2 3 3 3 4 4 4
        idx2 = np.tile(np.arange(len(segments2)), len(segments1))  # build indexes 1 2 3 1 2 3 1 2 3 1 2 3
        segment_pairs = np.concatenate((segments1[idx1], segments2[idx2]), axis=1)  # concatenate to create all pairs
        ...
        C = segment_pairs[:, 2]
        D = segment_pairs[:, 3]
        if np.logical_and(ccw(A, C, D) != ccw(B, C, D), ccw(A, B, C) != ccw(A, B, D)).any():
            n_collisions[veh1] += 1
            n_collisions[veh2] += 1
...
def _intesection_metrics_impl(trajectory_path, model, configs):
    ...
    for i in range(n_stacked_states - 1):
        for veh in controlled_vehicles:
            ...
            visible_state = scenario.flattened_visible_state(veh, view_dist=view_dist, view_angle=view_angle)
            ...
            state = (
                np.concatenate(
                    (
                        ...
                        scenario.flattened_visible_state(veh, view_dist=view_dist, view_angle=view_angle),
                    )
                )
                / state_normalization
            )
...
def compute_metrics_by_intersection(trajectories_dir, model, configs):
    ...
    with Pool(processes=NUM_CPUS) as pool:
        result = np.array(
            list(pool.starmap(_intesection_metrics_impl, zip(trajectories_paths, repeat(model), repeat(configs))))
        )
        assert result.shape == (len(trajectories_paths), 2, 4)  # collision rates, goal rates (in 4 bins)
