...
class BaseEnv(Env):
    ...
    def __init__(self, cfg: Dict[str, Any], rank: int = 0) -> None:
        ...
        self.seed(cfg["seed"])
        obs_dict = self.reset()
        self.observation_space = Box(low=-np.infty, high=np.infty, shape=(obs_dict[list(obs_dict.keys())[0]].shape[0],))
        if self.cfg["discretize_actions"]:
            ...
            self.action_space = Discrete(
                self.accel_discretization * self.steering_discretization * self.head_angle_discretization
            )
            self.accel_grid = np.linspace(
                -np.abs(self.cfg["accel_lower_bound"]), self.cfg["accel_upper_bound"], self.accel_discretization
            )
    ...
    def step(self, action_dict: Dict[int, Union[Action, np.ndarray, Sequence[float], int]]) -> None:
        ...
        for veh_obj in self.controlled_vehicles:
            ...
            if done_dict[veh_id]:
                ...
                if (info_dict[veh_id]["goal_achieved"] and self.cfg.get("remove_at_goal", True)) or (
                    info_dict[veh_id]["collided"] and self.cfg.get("remove_at_collide", True)
                ):
    ...
    def reset(self):
        ...
        while not enough_vehicles:
            ...
            self.simulation = Simulation(
                os.path.join(self.cfg["scenario_path"], self.file), config=get_scenario_dict(self.cfg)
            )
            self.scenario = self.simulation.getScenario()
            """##################################################################
            ...
            for veh_obj in self.simulation.getScenario().getObjectsThatMoved():
                ...
                if norm < self.cfg["rew_cfg"]["goal_tolerance"] or veh_obj.getCollided():
                    self.scenario.removeVehicle(veh_obj)
                """############################################
            ...
            for vehicle in temp_vehicles:
                ...
                elif not vehicle.expert_control and curr_index < self.cfg["max_num_vehicles"]:
    ...
    def get_observation(self, veh_obj):
        ...
        if self.cfg["subscriber"]["use_ego_state"] and self.cfg["subscriber"]["use_observations"]:
            obs = np.concatenate(
                (
                    ego_obs,
        ...
        elif self.cfg["subscriber"]["use_ego_state"] and not self.cfg["subscriber"]["use_observations"]:
            obs = ego_obs
        else:
